...
class Motor_de_inferencia:
    ...
    def __init__(self, model_xml : str, model_bin : str, device : str, confidence_threshold : float):
        ...
        # Load OpenVINO model
        _model = self.ie_core.read_model(model=model_xml, weights=model_bin)
        if _model:
            ...
            # self.input_blob_prop = next(iter(_model.input_info))
            # _model.batch_size = 1
            self.compiled_model = self.ie_core.compile_model(
                _model, device.upper()
            )
    ...
    def get_output_blob_prop(self) -> list:
        return next(iter(self.compiled_model.outputs))
